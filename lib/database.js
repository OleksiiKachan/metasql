...
class InsertQuery {
  ...
  doUpdate(exclude) {
    ...
    if (exclude) {
      ...
      this.notUpdateFields = fields.map((field) =>
  ...
  then(resolve, reject) {
    ...
    if (this.conflictFields && this.conflictAction) {
      sql.push(
        `ON CONFLICT (${this.conflictFields
          .map((field) => `"${field}"`)
          .join(', ')})`
      );
      ...
      if (this.conflictAction === 'nothing') {
        ...
        if (this.notUpdateFields?.length) {
          delta = Object.keys(this.record).reduce((record, fieldKey) => {
            if (!this.notUpdateFields.includes(fieldKey)) {
